...
public class MovablePointsSet {
    ...
    public MovablePointsSet() {
        for (int i = 0; i < 5; i++) {
            points.add(setRandomValue(maxCoordinate, minCoordinate, maxSpeed, minSpeed));   // сколько раз передвигать
        }
    }

    public MovablePointsSet(int numberOfPoint) {
        for (int i = 0; i < numberOfPoint; i++) {
            points.add(setRandomValue(maxCoordinate, minCoordinate, maxSpeed, minSpeed));   // количество точек
        }
    }
